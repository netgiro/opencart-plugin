...
*/
		...
		if (version_compare(VERSION, '2.0', '>=')) { // v2.0.x Compatibility
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/netgiro_post.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/payment/netgiro_post.tpl', $data);
			} else {
				return $this->load->view('extension/payment/netgiro_post.tpl', $data);
			}
	...
	public function callback() {
		...
		if (isset($_REQUEST['confirmationCode'])) {
			...
			if ($order_info) {
				$this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id = '" . (int)$this->config->get('netgiro_post_order_status_id') . "' WHERE order_id = '" . (int)$order_id . "'");								
			}	
