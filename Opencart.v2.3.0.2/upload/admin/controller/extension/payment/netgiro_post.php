...
class ControllerExtensionPaymentNetgiroPost extends Controller {
	...
	public function index() {
		...
		$logger = new Log('netgiro_debug.log'); 
		$logger->write('Debug: NETGIRO INDEX' . PHP_EOL); 
		...
		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
       		'href'      => $this->url->link('extension/' . $extension_type . '/' . $classname, 'token=' . $this->session->data['token'], 'SSL'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('extension/payment/netgiro_post', 'token=' . $this->session->data['token'], true);
		...
		include(DIR_APPLICATION . strtolower(get_parent_class($this)) . '/' . 'extension' . '/' . $extension_type . '/' . $classname . '.inc');
	...
	private function validate($errors = array()) {
		...
		if (!$this->user->hasPermission('modify', 'extension/' . $extension_type. '/' . $classname)) {		
